...
The plugin defaults to `https://learnmcp.dev/mcp`. For your own deployment, either:
...
- **Migrations 0001/0002 are partly vestigial.** Their `cartridges` and `user_profiles`
  ...
  the dead tables.
- **The cloud path has never run against a real Supabase.** It's covered by tests against a
  faked PostgREST — badge award, no double-granting, scan idempotency, `gte` counting — but
  the SQL in 0003 has not been executed. Run it on a scratch project first.
